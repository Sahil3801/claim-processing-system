...
const claimIds = new SharedArray('benchmark claim ids', () => {
  const values = JSON.parse(open(__ENV.CLAIM_IDS_FILE || '/results/claim-ids.json'));
...
export default function (data) {
  ...
  const hotsetSize = Math.max(1, Math.floor(claimIds.length * configuredHotset / 100));
  const selector = (exec.vu.idInTest * 104729 + exec.scenario.iterationInTest * 7919);
